const data = [
  ...
  },
  {
    ...
    category: "Frontend Users",
    description: "",
    code: `import React, { useContext, useEffect } from "react"; import { Context } from "../App"; import axios from "axios"; import { useNavigate } from "react-router-dom"; const Home = () => { const [globalEmail, setGlobalEmail] = useContext(Context); const navigate = useNavigate(); const handleLogout = () => { axios.post("http://localhost:3001/users/logout").then((response) => { console.log(response); }); navigate("/"); }; useEffect(() => { axios.get("http://localhost:3001/users/login").then((response) => { if (response.data.loggedIn && response.data.user) { setGlobalEmail(response.data.user.email); } else { navigate("/"); } }); }, []); return ( <div> <button className="fixed top-5 left-10 p-2 border-[2px] border-gray-800 rounded-lg" onClick={() => { handleLogout(); }}> Logout </button> <h1>Welcome {globalEmail}</h1> </div> ); }; export default Home;`,
    img: "/react-helper/images/blank.png",
  },
  {
    title: "Login.jsx",
  ...
  },
  {
    ...
    category: "Frontend Users",
    description: "",
    code: `import React, { createContext, useState } from "react"; import { BrowserRouter as Router, Routes, Route } from "react-router-dom"; import Users from "./pages/Users"; import Login from "./pages/Login"; import Register from "./pages/Register"; import Home from "./pages/Home"; export const Context = createContext(); const App = () => { const [globalEmail, setGlobalEmail] = useState(""); return ( <Context.Provider value={[globalEmail, setGlobalEmail]}> <Router> <Routes> <Route path="/" exact element={<Login />} /> <Route path="/register" exact element={<Register />} /> <Route path="/users" exact element={<Users />} /> <Route path="/home" exact element={<Home />} /> </Routes> </Router> </Context.Provider> ); }; export default App;`,
    img: "/react-helper/images/blank.png",
  },
];
